...
st.set_page_config(
    page_title="BAZI Tennis V5.4 VERIFIED",
    page_icon="🎾",
    layout="wide",
)
...
def get_player_current_season_fixtures(player_key):
    """
    Fetch current-season completed singles for one player.
    ...
    - must have a winner
    - dedupe by event_key
    - exclude walkovers/cancelled/abandoned records
    - keep retirements for match W/L, but they are flagged and excluded
      from set-transition statistics later
    """
    if not player_key:
        return []
    ...
    for m in rows if isinstance(rows, list) else []:
        ...
        status = str(m.get("event_status", "") or "").strip().lower()
        if status != "finished":
            continue
        ...
        final_result = str(m.get("event_final_result", "") or "").upper()
        raw_text = " ".join(
            str(m.get(k, "") or "")
            for k in ("event_status", "event_final_result", "event_game_result")
        ).upper()

        if any(x in raw_text for x in ("W/O", "WALKOVER", "CANCEL", "ABANDON")):
            continue
        ...
        m = dict(m)
        m["_bazi_retirement"] = any(x in raw_text for x in ("RET", "RETIRED"))
        out.append(m)

    return out


def season_fixture_audit_rows(player_key):
    """Human-readable rows for exactly the 2026 fixtures BAZI used."""
    fixtures = get_player_current_season_fixtures(player_key)
    pkey = str(player_key)

    rows = []
    for m in fixtures:
        first_key = str(m.get("first_player_key", ""))
        second_key = str(m.get("second_player_key", ""))
        is_first = first_key == pkey
        ...
        rows.append({
            ...
            "Score (player view)": _normalized_match_score_for_player(m, pkey),
            "Raw API score": m.get("event_final_result", "—"),
            "Player side": "First" if is_first else "Second",
            "Status": m.get("event_status", "—"),
            "Retirement": "Yes" if m.get("_bazi_retirement") else "No",
            "Consistency": _score_result_consistency(m, pkey),
            "Event key": m.get("event_key", "—"),
        })
...
def _normalized_match_score_for_player(match, player_key):
    """
    Return the final match score from the selected player's perspective.

    API-Tennis event_final_result is oriented First Player - Second Player.
    If the selected player is second, reverse it.
    """
    raw = str(match.get("event_final_result", "") or "").strip()
    if not raw or "-" not in raw:
        return raw or "—"

    try:
        left, right = [x.strip() for x in raw.split("-", 1)]
        first_key = str(match.get("first_player_key", ""))
        second_key = str(match.get("second_player_key", ""))
        pkey = str(player_key)
...
def _normalized_set_scores_for_player(match, player_key):
    """
    Return set scores from the selected player's perspective as
    ...
    out = []
    for s in match.get("scores", []) if isinstance(match.get("scores"), list) else []:
        try:
        ...
        if pkey == first_key:
            ...
            # Unknown orientation: skip instead of guessing.
            continue
...
def _season_fixture_profile(player_key):
    ...
    for m in fixtures:
        ...
        # Retirements count for match W/L but not for set-transition stats,
        # because incomplete sets can distort Set 1/2/3 percentages.
        if m.get("_bazi_retirement"):
            if is_first:
                opp = m.get("event_second_player", "Opponent")
            else:
                opp = m.get("event_first_player", "Opponent")

            recent.append({
                "date": m.get("event_date", ""),
                "result": "W" if won_match else "L",
                "score": _normalized_match_score_for_player(m, pkey),
                "opponent": opp,
            })
            continue

        for pg, og in _normalized_set_scores_for_player(m, pkey):
            won_set = pg > og
...
def render_live_match_details(match):
    ...
    with st.expander("🔎 Verify 2026 source matches", expanded=False):
        render_2026_verification(match)
    ...
    if match.get("first_player_key") and match.get("second_player_key"):
        if st.button(
            ...
            try:
                ...
                render_set_prediction(match, result)
...
def verified_last10_summary(results):
    """Summary from the exact audited 2026 fixture list used by BAZI."""
    if not results:
        return "—"
    ...
    streak = 0
    first = rows[0].get("result")
    if first in {"W", "L"}:
        for r in rows:
            if r.get("result") == first:
                streak += 1
            else:
                break

    return f"{wins}-{losses} ({first}{streak})" if streak else f"{wins}-{losses}"
...
def deep_matchup_table(match):
    ...
    # Use the same audited completed-fixture list for Last 10 and season stats.
    # This prevents mismatches where one table used H2H endpoint history while
    # another used the full player fixture history.
    api1 = season1.get("last10", [])
    api2 = season2.get("last10", [])

    rows = [
        ("Last 10 (verified 2026)", verified_last10_summary(api1), verified_last10_summary(api2)),
        ("2026 match win % (verified)", _fmt_pct(*season1["win_rate"]), _fmt_pct(*season2["win_rate"])),
        ...
        ("2024-25 matches in local DB", hp1.get("matches", 0) or "—", hp2.get("matches", 0) or "—"),
...
def render_last10_cards(p1, p2, hp1, hp2, api1, api2):
    ...
    def render_side(col, player, api_rows, hist_profile):
        with col:
            ...
            for r in rows[:10]:
                icon = "🟢 W" if r.get("result") == "W" else "🔴 L"
                rank = f" #{r['rank']}" if r.get("rank") else ""
                date_txt = f" · {r.get('date')}" if r.get("date") else ""
                st.markdown(
                    f"{icon}  **{r.get('score','—')}**  "
                    f"{r.get('opponent','Opponent')}{rank}{date_txt}"
                )
...
def render_deep_analysis(match):
    ...
    st.caption(
        "2026 rows and Last 10 now come from the SAME verified completed-match "
        "fixture list. 2024–25 rows come from your local ATP/WTA CSV database. "
...
def _get_set_rate_from_profile(profile, set_no):
    key = f"set{set_no}"
    value = profile.get(key)
...
def _set_pair_probability(rate1, rate2, prior_n=8):
    """Sample-size-adjusted probability that Player 1 wins the set."""
    a, an = _shrunk_pct(rate1, prior_n=prior_n)
    b, bn = _shrunk_pct(rate2, prior_n=prior_n)
    ...
    p1 = _pairwise_rate(a, b)
    return p1, min(an, bn)


def set_win_prediction(match, base_result=None):
    """
    Predict the current set (live) or Set 1 (upcoming).

    Upcoming:
      current-season set record + 2024-25 set record + rank/current form.

    Live:
      adds current-set game score, live stats, server, and reduces pre-match weight.
    """
    p1_key = match.get("first_player_key")
    p2_key = match.get("second_player_key")
    p1_name = match.get("event_first_player") or "Player 1"
    ...
    if base_result is None:
        base_result = analyze_match(match)

    season1 = base_result.get("season1") or _season_fixture_profile(p1_key)
    season2 = base_result.get("season2") or _season_fixture_profile(p2_key)
    hp1 = base_result.get("historical1") or historical_profile(
        load_historical_matches(), p1_name, current_surface_name(match)
    )
    hp2 = base_result.get("historical2") or historical_profile(
        load_historical_matches(), p2_name, current_surface_name(match)
    )

    p1_sets, p2_sets, _, _ = _set_and_game_state(match)
    live_now = match_state(match) == "LIVE"
    current_set = p1_sets + p2_sets + 1 if live_now else 1
    current_set = max(1, min(3, current_set))

    # Current-season set-specific signal.
    season_rate1 = _get_set_rate_from_profile(season1, current_set)
    season_rate2 = _get_set_rate_from_profile(season2, current_set)
    p_season, set_sample = _set_pair_probability(season_rate1, season_rate2, prior_n=8)

    # 2024-25 set-specific signal.
    hist_rate1 = _get_set_rate_from_profile(hp1, current_set)
    hist_rate2 = _get_set_rate_from_profile(hp2, current_set)
    p_hist, _ = _set_pair_probability(hist_rate1, hist_rate2, prior_n=10)

    # Base match-strength prior.
    p_base = float(base_result.get("p1_prob", 0.5))

    # Live match inputs.
    p1_games, p2_games = _current_set_score(match) if live_now else (0, 0)
    game_diff = p1_games - p2_games
    stat_adv = _live_stat_advantage(match) if live_now else 0.0

    serve = str(match.get("event_serve", "") or "")
    serve_edge = 0.0
    if live_now:
        serve_edge = 0.035 if serve == "First Player" else (-0.035 if serve == "Second Player" else 0.0)

    if live_now:
        # Current set score is the strongest signal, then live stats, then set history.
        logit_base = math.log(max(0.03, min(0.97, p_base)) / (1 - max(0.03, min(0.97, p_base))))
        season_logit = math.log(max(0.03, min(0.97, p_season)) / (1 - max(0.03, min(0.97, p_season))))
        hist_logit = math.log(max(0.03, min(0.97, p_hist)) / (1 - max(0.03, min(0.97, p_hist))))

        score_signal = max(-1.35, min(1.35, 0.32 * game_diff))
        live_signal = 0.85 * stat_adv
        ...
        p1_set = 1.0 / (1.0 + math.exp(-set_logit))
    else:
        # Pre-match Set 1.
        p1_set = (
            0.45 * p_season
    ...
    p1_set = max(0.08, min(0.92, p1_set))
    p2_set = 1.0 - p1_set

    pick = p1_name if p1_set >= p2_set else p2_name
    confidence = max(p1_set, p2_set)
...
def render_set_prediction(match, base_result=None):
    sp = set_win_prediction(match, base_result)

    title = (
        f"🎯 CURRENT SET {sp['set_no']} WIN PREDICTION"
        if sp["live"]
        else "🎯 SET 1 WIN PREDICTION"
    )
    st.markdown(f"### {title}")
    ...
    c1, c2 = st.columns(2)
    c1.metric(p1, f"{sp['p1_prob']*100:.1f}%")
    c2.metric(p2, f"{sp['p2_prob']*100:.1f}%")

    if sp["live"]:
        st.caption(
            f"Current set score: {sp['p1_games']}–{sp['p2_games']} • "
            f"Set-history sample floor: {sp['sample']}"
        )
    else:
        st.caption(
            f"Uses 2026 Set 1 record + 2024–25 Set 1 history + pre-match strength. "
            f"Set-history sample floor: {sp['sample']}"
        )

    if sp["confidence"] >= 0.62:
        st.success(
            f"Set lean: {sp['pick']} "
            f"({sp['confidence']*100:.1f}% set-win probability)"
        )
    else:
        st.info(
            f"Set lean: {sp['pick']} "
            f"({sp['confidence']*100:.1f}%) • low edge"
        )
...
def _score_result_consistency(match, player_key):
    """Check whether normalized match score agrees with event_winner."""
    score = _normalized_match_score_for_player(match, player_key)
    if not score or "-" not in score:
        return "Unknown"

    try:
        a, b = [int(x.strip()) for x in score.split("-", 1)]
    except Exception:
        return "Unknown"

    first_key = str(match.get("first_player_key", ""))
    pkey = str(player_key)
    winner = str(match.get("event_winner", ""))

    won = (
        (pkey == first_key and winner == "First Player")
        or (pkey != first_key and winner == "Second Player")
    )
    ...
    score_says_won = a > b
    return "OK" if score_says_won == won else "CHECK"


def render_2026_verification(match):
    """Show exactly which API-Tennis fixtures produced the 2026 stats."""
    p1 = match.get("event_first_player") or "Player 1"
    p2 = match.get("event_second_player") or "Player 2"
    p1_key = match.get("first_player_key")
    p2_key = match.get("second_player_key")

    st.markdown("### 🔎 VERIFY 2026 DATA")
    st.caption(
        "These are the exact completed singles fixtures BAZI used from API-Tennis. "
        "Walkovers/cancelled matches are excluded. Retirements count for match W/L "
        "but are excluded from Set 1/2/3 transition statistics."
    )

    left, right = st.columns(2)

    for col, player, key in ((left, p1, p1_key), (right, p2, p2_key)):
        with col:
            rows = season_fixture_audit_rows(key)
            st.markdown(f"**{player} — {len(rows)} verified 2026 matches**")

            if not rows:
                st.warning("No verified 2026 completed singles were returned.")
                continue

            wins = sum(r["Result"] == "W" for r in rows)
            losses = len(rows) - wins
            retirements = sum(r["Retirement"] == "Yes" for r in rows)

            a, b, c = st.columns(3)
            a.metric("W", wins)
            b.metric("L", losses)
            c.metric("RET", retirements)

            st.dataframe(
                pd.DataFrame(rows),
                hide_index=True,
                use_container_width=True,
                height=min(420, 42 + 35 * min(len(rows), 10)),
            )
...
# UI
# -----------------------------
st.title("🎾 BAZI Tennis V5.4 VERIFIED")
st.caption("API-Tennis live feed • match win prediction + set win prediction • normalized/auditable 2026 data")
