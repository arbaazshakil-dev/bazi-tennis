...
def live_adjusted_prediction(match, base_result):
    """
    Blend the pre-match-style BAZI probability with current match state.

    This is intentionally transparent and bounded:
    sets > games > live stats > current server.
    """
    base_p = float(base_result["p1_prob"])
    base_p = min(0.98, max(0.02, base_p))
    base_logit = math.log(base_p / (1.0 - base_p))

    p1_sets, p2_sets, p1_games, p2_games = _set_and_game_state(match)
    set_diff = p1_sets - p2_sets
    game_diff = p1_games - p2_games

    # Match-state influence.
    set_signal = 0.95 * set_diff
    game_signal = max(-0.55, min(0.55, 0.075 * game_diff))

    # Live-stat influence.
    stat_adv = _live_stat_advantage(match)
    stat_signal = 0.55 * stat_adv

    # Small server edge only; never let serving dominate the model.
    serve = str(match.get("event_serve", "") or "")
    serve_signal = 0.05 if serve == "First Player" else (-0.05 if serve == "Second Player" else 0.0)

    live_logit = base_logit + set_signal + game_signal + stat_signal + serve_signal
    p1_live = 1.0 / (1.0 + math.exp(-live_logit))
    p1_live = min(0.97, max(0.03, p1_live))
    p2_live = 1.0 - p1_live
    ...
    if match.get("first_player_key") and match.get("second_player_key"):
        if st.button(
            ...
            try:
                ...
                st.caption("Live BAZI updates with the current set/game score, selected live statistics, and server. It is an analytical estimate, not a guaranteed outcome.")
...
def resolve_historical_name(api_name, hist):
    """Resolve API abbreviations such as 'N. Borges' to CSV full names."""
    ...
    for n in pool:
        ...
        if nparts[-1] == last:
            score = 2.0
            if first_initial and nparts[0].startswith(first_initial):
                score += 2.0
            score += difflib.SequenceMatcher(None, api_clean, nc).ratio()
    ...
    close = difflib.get_close_matches(api_name, pool, n=1, cutoff=0.72)
...
def deep_matchup_table(match):
    ...
    rows = [
        ...
        ("2026 win %", _fmt_pct(s1["win_rate"] * 100, s1["wins"] + s1["losses"]), _fmt_pct(s2["win_rate"] * 100, s2["wins"] + s2["losses"])),
...
# UI
# -----------------------------
st.title("🎾 BAZI Tennis V4.0")
st.caption("API-Tennis live feed • deep 2024–2026 matchup analysis • live win probability")
