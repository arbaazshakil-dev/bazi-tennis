    ...
    if match.get("first_player_key") and match.get("second_player_key"):
        if st.button(
            ...
            try:
                ...
                x, y = st.columns(2)
                x.metric(result["p1_name"], f"{result['p1_prob'] * 100:.1f}%")
                y.metric(result["p2_name"], f"{result['p2_prob'] * 100:.1f}%")
                ...
                st.caption("BAZI confidence is an analytical estimate, not a guaranteed outcome.")
...
with tabs[0]:
    ...
    if live:
        ...
        live_by_key = {str(m.get("event_key")): m for m in live}
        ...
        selected_key = st.session_state.get("selected_live_match")
        if selected_key in live_by_key:
            render_live_match_details(live_by_key[selected_key])
        elif selected_key:
